...
# neighborhood
pred = read.table("projection_neigbor_rf_temp.txt", h=T)
head(pred)
...
# Subset the data, stratified sampling
#bins = floor(data$annual_mean_temp)
#dens.bins = table(bins)/nrow(data)
#dens.tp = numeric(nrow(data))
#for(i in 1:length(table(bins))) dens.tp[bins==names(table(bins))[i]]=dens.bins[i]
#ID = c(1:nrow(data))
#subID = sample(ID,size = 10000,prob = dens.tp)
#data = data[subID,]
#pred = pred[subID,]

# Subset the data, plots with low precipitation and high temp
subID = numeric(nrow(data))
subID[data$annual_mean_temp < 15] = 1
subID[data$annual_pp < 700] = 0
data = data[subID==1,]
pred = pred[subID==1,]

#check variables
data$ENV1 = (data$annual_mean_temp-1.529991)/1.891697
data$ENV2 = (data$annual_pp-1044.194)/132.858
data$EB = pred$B
data$ET = pred$T
data$EM = pred$M 
