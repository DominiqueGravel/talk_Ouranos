...
sample_per_bin = sample_size/length(table(bins))
prob.bins = sample_per_bin/table(bins)
prob.bins[prob.bins>1]=1
prob.tp = numeric(nrow(data))
for(i in 1:length(table(bins))) prob.tp[bins==names(table(bins))[i]]=prob.bins[i]
ID = c(1:nrow(data))
subID = sample(ID,size = 10000,prob = prob.tp)
data = data[subID,]
pred = pred[subID,]
